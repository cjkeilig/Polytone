...
if __name__ == '__main__':
    ...
    parser = OptionParser (usage='%prog [-h] [-u] [-m] [-c C] [-d D] [-i I] [-n CPL] [-b BPL] [-o DIR] [-v V] [-x] [-p PFMT] <file1> [<file2> ...]', version=str(VERSION))
    ...
    parser.add_option ("-i", action="store", help="ignore number of lines from the beginning", default=0)
    ...
    for X, fnmext in enumerate (fnmext_list):
        ...
        if ext.lower () == '.mxl':          # extract .xml file from .mxl file
            ...
            if options.i > 0:
                fobj = open (fnmext).seek(int(options.i))
            else:
